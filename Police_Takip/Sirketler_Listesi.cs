...
{
    public partial class Sirketler_Listesi : UserControl
    {
        Database_Control dc = new Database_Control();
        int selected_id = 0;
        public Sirketler_Listesi()
        {
            InitializeComponent();
        }
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            if (dc.Check_Data_Exists("sirketler", "data", guna2TextBox1.Text)) { MessageBox.Show("Aynı veri zaten mevcut", dc.app_name, MessageBoxButtons.OK, MessageBoxIcon.Error); }
            else
            {
                if (dc.Insert_Data("sirketler", "'data'", "'" + guna2TextBox1.Text + "'") != -1)
                {
        ...
        {
            guna2DataGridView1.DataSource = dc.GetTableFromDatabase("sirketler");
        ...
        private void guna2Button3_Click(object sender, EventArgs e)
        {
            ...
            {
                if (dc.Delete_Data("sirketler", "id", selected_id.ToString()) == true)
                {
        ...
        private void guna2Button2_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (result == DialogResult.Yes)
                {
                    if (dc.Update_Data("sirketler", "id", selected_id.ToString(), "'data'", guna2TextBox1.Text) == true)
                    {
        ...
        private void Sirketler_Listesi_Load(object sender, EventArgs e)
        {
            refresh_dataviewer();
        }
    }
